...
SuffixArray::SuffixArray(const std::string &base_name, bool print)
    : ok(false), data(0), sa(0), original_sa(0), ranks(0), data_size(0),
      sequences(0) {
  std::ifstream data_file(base_name.c_str(), std::ios_base::binary);
  if (!data_file) {
    std::cerr << "Error: Cannot open data file " << base_name << "!"
              << std::endl;
    return;
  }
  ...
  if (!sa_file) {
    std::cerr << "Error: Cannot open suffix array file " << sa_name << "!"
              << std::endl;
    return;
  }
  ...
  this->original_sa = this->sa;
  this->sa += this->sequences;
  this->ok = true;
}

SuffixArray::SuffixArray(uchar *_data, uint bytes, uint threads)
    : ok(false), data(_data), sa(0), original_sa(0), ranks(0), data_size(bytes),
      sequences(0) {
  if (_data == 0 || bytes == 0) {
    std::cerr << "Error: No input data given for suffix array construction!"
              << std::endl;
    return;
  }
  ...
  short_pair *pairs =
      simpleSuffixSort(this->data, this->data_size, this->sequences, threads);
  this->sa = new uint[this->data_size];
  for (uint i = 0; i < this->data_size; i++) {
    this->sa[i] = pairs[i].first;
  }
  delete[] pairs;
...
SuffixArray::SuffixArray(uchar *_data, usint *ra, uint bytes, uint threads)
    : ok(false), data(_data), sa(0), original_sa(0), ranks(ra),
      data_size(bytes), sequences(0) {
  ...
  for (usint i = 0; i < this->data_size; i++) {
    pairs[i] = pair_type(this->ranks[i], i);
  }
  omp_set_num_threads(threads);
  parallelSort(pairs, pairs + this->data_size);
  this->sa = new uint[this->data_size];
  for (uint i = 0; i < this->data_size; i++) {
    this->sa[i] = pairs[i].second;
  }
  delete[] pairs;
...
void SuffixArray::writeTo(const std::string &base_name, bool write_data) const {
  if (!this->ok) {
    return;
  }
...
pair_type SuffixArray::count(const std::string &pattern) const {
  pair_type range;
  ...
  while (low < high) {
    ...
    if (matched >= pattern.length()) {
      high = mid;
    } else if ((uchar)(pattern[matched]) <
               this->data[this->sa[mid] + matched]) {
      high = ((sint)mid) - 1;
      last_high = high;
    } else {
      low = mid + 1;
  ...
  if (this->match(pattern, low) == pattern.length()) {
    range.first = low;
  } else {
    return EMPTY_PAIR;
  }
...
uint SuffixArray::match(const std::string &pattern, uint index) const {
  uint matched = 0;
  for (uint pos = this->sa[index]; matched < pattern.length() &&
                                   (uchar)(pattern[matched]) == this->data[pos];
       matched++, pos++)
    ;
  return matched;
}

uint *SuffixArray::locate(pair_type range) const {
  if (isEmpty(range) || range.second >= this->getSize()) {
    return 0;
  }

  uint *result = new uint[length(range)];
  for (usint i = range.first; i <= range.second; i++) {
    result[i - range.first] = this->sa[i];
  }
  return result;
}

uint SuffixArray::locate(uint index) const {
...
uint *SuffixArray::getLCPArray(bool getPLCP) const {
  if (!(this->isOk())) {
    return 0;
  }
  ...
  for (uint i = this->sequences; i < this->data_size; i++) {
    ...
    if (j == 0 || k == 0 || this->data[j - 1] != this->data[k - 1] ||
        this->data[j - 1] == 0) {
      uint temp = 0;
      while (this->data[j + temp] == this->data[k + temp] &&
             this->data[j + temp] != 0) {
        temp++;
      }
      plcp[k] = temp;
...
usint SuffixArray::reportSize(bool print) const {
  usint bytes = sizeof(*this) + this->data_size * (1 + sizeof(uint));
  if (this->ranks != 0) {
    bytes += this->data_size + sizeof(usint);
  }
  if (print) {
    std::cout << "Data:            " << (this->data_size / (double)MEGABYTE)
              << " MB" << std::endl;
    std::cout << "Suffix array:    "
              << (this->data_size * sizeof(uint) / (double)MEGABYTE) << " MB"
              << std::endl;
    if (this->ranks != 0) {
      std::cout << "Rank array:      "
                << (this->data_size * sizeof(usint) / (double)MEGABYTE) << " MB"
                << std::endl;
    }
    std::cout << "Total size:      " << (bytes / (double)MEGABYTE) << " MB"
              << std::endl;
    std::cout << std::endl;
  }
  return bytes;
