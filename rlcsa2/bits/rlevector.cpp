...
RLEVector::RLEVector(std::ifstream &file) : BitVector(file) {}

RLEVector::RLEVector(FILE *file) : BitVector(file) {}
...
usint RLEVector::reportSize() const {
...
usint RLEVector::Iterator::rank(usint value, bool at_least) {
...
usint RLEVector::Iterator::select(usint index) {
  const RLEVector &par = (const RLEVector &)(this->parent);
...
usint RLEVector::Iterator::selectNext() {
...
pair_type RLEVector::Iterator::valueBefore(usint value) {
  const RLEVector &par = (const RLEVector &)(this->parent);

  if (value >= par.size) {
    return pair_type(par.size, par.items);
  }
...
pair_type RLEVector::Iterator::valueAfter(usint value) {
  const RLEVector &par = (const RLEVector &)(this->parent);
...
pair_type RLEVector::Iterator::nextValue() {
  if (this->cur >= this->block_items) {
    this->getSample(this->block + 1);
    this->run = 0;
...
pair_type RLEVector::Iterator::selectRun(usint index, usint max_length) {
...
pair_type RLEVector::Iterator::selectNextRun(usint max_length) {
...
bool RLEVector::Iterator::isSet(usint value) {
  const RLEVector &par = (const RLEVector &)(this->parent);
...
usint RLEVector::Iterator::countRuns() {
  const RLEVector &par = (const RLEVector &)(this->parent);
  ...
  while (last < par.size) {
    ...
    if (res.first < par.size && res.first > last + 1) {
      runs++;
    }
...
void RLEVector::Iterator::valueLoop(usint value) {
  this->getSample(this->sampleForValue(value));
  this->run = 0;
...
RLEEncoder::RLEEncoder(usint block_bytes, usint superblock_size)
    : VectorEncoder(block_bytes, superblock_size), run(EMPTY_PAIR) {}

RLEEncoder::~RLEEncoder() {}

void RLEEncoder::setBit(usint value) { this->setRun(value, 1); }

void RLEEncoder::setRun(usint start, usint len) {
  if (this->items == 0) {
  ...
  if (free_bits > code_bits) // At least a part of the run fits into the block.
  {
    ...
    usint llen = 1;
    while (llen + 2 * length(llen + 1) - 1 <= free_bits) {
      llen++;
    }
    llen = ((usint)1 << llen) - 1;
...
void RLEEncoder::addBit(usint value) { this->addRun(value, 1); }

void RLEEncoder::addRun(usint start, usint len) {
