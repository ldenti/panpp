...
void largeWrite(std::ofstream &file, char *data, std::streamoff size,
                std::streamoff block_size) {
...
void readRows(std::ifstream &file, std::vector<std::string> &rows,
              bool skipEmptyRows) {
  while (file) {
    std::string buf;
    std::getline(file, buf);
    if (skipEmptyRows && buf.length() == 0) {
      continue;
    }
    rows.push_back(buf);
  }
...
double readTimer() { return omp_get_wtime(); }
...
template <class T>
uint setRanks(T *pairs, uint *keys, uint n, std::vector<ss_range> &unsorted,
              uint threads, uint chunk) {
  uint total = 0;
...
uint initialSort(short_pair *pairs, uint *keys, std::vector<ss_range> &unsorted,
                 uint n, uint threads, uint h) {
  ...
  //  std::cout << "Sorted with h = " << h << ", unsorted total = " << total <<
  //  " (" << unsorted.size() << " ranges)" << std::endl;

  return total;
}

short_pair *packPairs(skew_pair *old_pairs, uint n) {
...
short_pair *prefixDoubling(short_pair *pairs, uint *keys,
                           std::vector<ss_range> &unsorted, uint n,
...
#pragma omp parallel for schedule(dynamic, chunk)
    for (uint i = 0; i < unsorted.size(); i++) {
      ...
      for (short_pair *curr = pairs + unsorted[i].first; curr <= limit;
           ++curr) {
        curr->second = keys[curr->first + h];
      }
      sequentialSort(pairs + unsorted[i].first, pairs + unsorted[i].second + 1,
                     key_comparator);
    }
...
short_pair *prefixTripling(skew_pair *pairs, uint *keys,
                           std::vector<ss_range> &unsorted, uint n,
                           uint threads, uint total, uint h) {
...
#pragma omp parallel for schedule(dynamic, chunk)
    for (uint i = 0; i < unsorted.size(); i++) {
      ...
      for (skew_pair *curr = pairs + unsorted[i].first; curr <= limit; ++curr) {
        ...
        if (n - h > curr->first + h) {
          curr->second += keys[curr->first + 2 * h];
        }
      }
      sequentialSort(pairs + unsorted[i].first, pairs + unsorted[i].second + 1,
                     skew_comparator);
...
short_pair *simpleSuffixSort(short_pair *pairs, uint n, uint threads) {
  if (pairs == 0 || n == 0) {
    return 0;
  }
...
short_pair *simpleSuffixSort(const uchar *sequence, uint n, uint sequences,
  ...
  /*  uint zeros = 0;
    ...
    {
      pairs[i].first = i;
      pairs[i].second = (sequence[i] == '\0' ? zeros++ : sequence[i] +
    sequences);
    }
    uint h = 1;
    if(length(CHARS + sequences - 1) <= sizeof(uint) * CHAR_BIT / 2)
    {
      ...
      {
        pairs[i].second = (CHARS + sequences) * pairs[i].second + pairs[i +
    1].second;
      }
...
void mergeRanges(std::vector<pair_type> *vec, bool parallel) {
  if (vec == 0 || vec->size() <= 1) {
    return;
  }
  if (parallel) {
    ...
    sequentialSort(vec->begin(), vec->end());
  }

  std::vector<pair_type>::iterator prev = vec->begin();
  for (std::vector<pair_type>::iterator curr = prev + 1; curr != vec->end();
       ++curr) {
    if (prev->second + 1 >= curr->first) {
      prev->second = std::max(curr->second, prev->second);
    } else {
      ++prev;
      *prev = *curr;
    }
