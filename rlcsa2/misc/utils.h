...
template <class T> struct GenericTriple {
  T first;
  T second;
  T third;
...
// This is a workaround to a bug in ext4 file system. Writes larger than 2
// gigabytes can fail, if done directly. Block size is in megabytes.
...
template <class Iterator> void parallelSort(Iterator first, Iterator last) {
...
void parallelSort(Iterator first, Iterator last, const Compare &comp) {
#ifdef MULTITHREAD_SUPPORT
#ifdef _GLIBCXX_PARALLEL
...
template <class Iterator> void sequentialSort(Iterator first, Iterator last) {
#ifdef MULTITHREAD_SUPPORT
#ifdef _GLIBCXX_PARALLEL
...
template <class Iterator, class Compare>
void sequentialSort(Iterator first, Iterator last, const Compare &comp) {
#ifdef MULTITHREAD_SUPPORT
#ifdef _GLIBCXX_PARALLEL
...
template <class T>
void removeDuplicates(std::vector<T> *vec, bool parallel = true) {
  if (vec == 0) {
    return;
  }
  if (parallel) {
